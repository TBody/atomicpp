...
c  the knots in the x-direction.
      mk1 = mx-kx1
      if(mk1.eq.0)print *, "BA_FPEGR 4"      
      if(mk1.eq.0) go to 60
      k3 = kx/2
      i = kx1+1
      j = k3+2
      if(k3*2.eq.kx)print *, "BA_FPEGR 5"      
...
c  the knots in the y-direction.
  60  mk1 = my-ky1
      if(mk1.eq.0) print *, "BA_FPEGR 6"  
      if(mk1.eq.0) go to 120
      k3 = ky/2
      i = ky1+1
      j = k3+2
      if(k3*2.eq.ky)print *, "BA_FPEGR 7"      
...
c  if s > 0 our initial choice of knots depends on the value of iopt.
      if(iopt.eq.0) print *, "BA_FPEGR 8"
 100  if(iopt.eq.0) go to 115
      if(fp0.le.s) print *, "BA_FPEGR 9" 
...
c  interval (tx(l),tx(l+1)).
      ...
      do 105 i=2,mpm
        nrdatx(j) = nrdatx(j)+1
      if(x(i).lt.tx(l)) print *, "BA_FPEGR 10"        
...
c  interval (ty(l),ty(l+1)).
      ...
      do 110 i=2,mpm
        nrdaty(j) = nrdaty(j)+1
      if(y(i).lt.ty(l)) print *, "BA_FPEGR 11"        
...
c  bound for the number of trials.
      do 250 iter=1,mpm
        if(nx.eq.nminx .and. ny.eq.nminy)print *, "BA_FPEGR 12"        
...
c  because of the storage capacity limitation.
        if(nx.eq.nxe .and. ny.eq.nye) print *, "BA_FPEGR 18"
...
c  in which the last added knots were located.
        if(lastdi.lt.0) print *, "BA_FPEGR 18.1"
        if(lastdi.lt.0) go to 150
        if(lastdi.eq.0) print *, "BA_FPEGR 18.2"
...
c  find nplx, the number of knots we should add in the x-direction.
        nplx = 1
        if(nx.eq.nminx) print *, "BA_FPEGR 19"        
        if(nx.eq.nminx) go to 180
        npl1 = nplusx*2
        rn = nplusx
        if(reducx.gt.acc) print *, "BA_FPEGR 20"        
...
c  find nply, the number of knots we should add in the y-direction.
 180    nply = 1
        if(ny.eq.nminy) print *, "BA_FPEGR 21" 
        if(ny.eq.nminy) go to 190
        npl1 = nplusy*2
        rn = nplusy
        if(reducy.gt.acc) print *, "BA_FPEGR 22"        
        if(reducy.gt.acc) npl1 = rn*fpms/reducy
        nply = min0(nplusy*2,max0(npl1,nplusy/2,1))
 190    if(nplx.lt.nply) print *, "BA_FPEGR 18.3"
 190    if(nplx.lt.nply) go to 210
        if(nplx.eq.nply) print *, "BA_FPEGR 18.4"
        if(nplx.eq.nply) go to 200
        go to 230
        if(lastdi.lt.0) print *, "BA_FPEGR 23"        
 200    if(lastdi.lt.0) go to 230
        if(nx.eq.nxe) print *, "BA_FPEGR 24" 
...
c  x-direction.
          if(nx.eq.nxe) print *, "BA_FPEGR 25"
          if(nx.eq.nxe) go to 250
 220    continue
        go to 250
        if(ny.eq.nye) print *, "BA_FPEGR 26"        
...
c  test whether we cannot further increase the number of knots in the
c  y-direction.
          if(ny.eq.nye) print *, "BA_FPEGR 27"
...
c  test whether the approximation sp(x,y) is an acceptable solution.
        fpms = fp-s
        if(abs(fpms).lt.acc) print *, "BA_FPEGR 29"        
        if(abs(fpms).lt.acc) go to 440
c  test whether the maximum allowable number of iterations has been
c  reached.
        if(iter.eq.maxit) print *, "BA_FPEGR 30"
        if(iter.eq.maxit) go to 400
c  carry out one more step of the iteration process.
        p2 = p
        f2 = fpms
        if(ich3.ne.0) print *, "BA_FPEGR 31"        
        if(ich3.ne.0) go to 320
        if((f2-f3).gt.acc) print *, "BA_FPEGR 32"        
        if((f2-f3).gt.acc) go to 310
c  our initial choice of p is too large.
        p3 = p2
        f3 = f2
        p = p*con4
        if(p.le.p1) print *, "BA_FPEGR 33"        
        if(p.le.p1) p = p1*con9 + p2*con1
        go to 350
        if(f2.lt.0.)print *, "BA_FPEGR 34"        
 310    if(f2.lt.0.) ich3 = 1
        if(ich1.ne.0) print *, "BA_FPEGR 35" 
 320    if(ich1.ne.0) go to 340
        if((f1-f2).gt.acc) print *, "BA_FPEGR 36" 
        if((f1-f2).gt.acc) go to 330
c  our initial choice of p is too small
        p1 = p2
        f1 = f2
        p = p/con4
        if(p3.lt.0.) print *, "BA_FPEGR 37"        
        if(p3.lt.0.) go to 350
        if(p.ge.p3) print *, "BA_FPEGR 38"        
        if(p.ge.p3) p = p2*con1 + p3*con9
        go to 350
c  test whether the iteration process proceeds as theoretically
c  expected.
        if(f2.gt.0.)print *, "BA_FPEGR 39"
 330    if(f2.gt.0.) ich1 = 1
        if(f2.ge.f1 .or. f2.le.f3) print *, "BA_FPEGR 40" 
